...
const { getCurrentFiberOwnerName, getCurrentFiberStackAddendum } = ReactDebugCurrentFiber;
...
if (__DEV__) {
  ...
  warnForTextDifference = function(serverText: string, clientText: string | number) {
  ...
  warnForPropDifference = function(propName: string, serverValue: mixed, clientValue: mixed) {
    if (didWarnInvalidHydration) {
      return;
    }
  ...
  normalizeHTML = function(parent: Element, html: string) {
    ...
    const testElement =
      parent.namespaceURI === HTML_NAMESPACE
        ...
        : parent.ownerDocument.createElementNS((parent.namespaceURI: any), parent.tagName);
  ...
  for (const propKey in rawProps) {
    ...
    if (propKey === CHILDREN) {
      ...
      if (suppressHydrationWarning) {
        ...
        const expectedStyle = CSSPropertyOperations.createDangerousStringForStyles(nextProp);
...
export function warnForDeletedHydratableElement(parentNode: Element | Document, child: Element) {
  if (__DEV__) {
    if (didWarnInvalidHydration) {
      return;
    }
...
export function warnForDeletedHydratableText(parentNode: Element | Document, child: Text) {
  if (__DEV__) {
    if (didWarnInvalidHydration) {
      return;
    }
...
export function warnForInsertedHydratedText(parentNode: Element | Document, text: string) {
  if (__DEV__) {
    if (text === '') {
